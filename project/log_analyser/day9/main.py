...
def read_logs():
    with open(SCRIPT_DIR / "logs.txt","r") as f:
        return f.readlines()

def filter_logs(logs: List[str], log_type: str) -> List[str]:
    if log_type == "error":
        return [log for log in logs if log.count("ERROR") > 0]
    elif log_type == "info":
        return [log for log in logs if log.count("INFO") > 0]
...
def log_analyser():
    ...
    total_logs = len(logs)
    total_errors = sum(1 for log in logs if log.count("ERROR")>0)
...
def logs(type: str = None, limit: int = None):
    ...
    return {"logs":[l.strip() for l in  filtered_logs]}
...
def error():
    logs = read_logs()
    errors = (log.strip() for log in logs if log.count("ERROR") > 0)
